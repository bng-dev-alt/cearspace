...
export async function POST(req: NextRequest) {
  try {
    ...
    const { projectName, columns, context } = body;

    if (!projectName) {
      return NextResponse.json(
        { error: 'Chybí název projektu pro analýzu rizik.' },
        { status: 400 }
      );
    }
    ...
    if (!columns || !Array.isArray(columns) || columns.length === 0) {
      return NextResponse.json(
        { error: 'Projekt nemá žádné sloupce ani úkoly k analýze.' },
        { status: 400 }
      );
    }

    const response = await aiService.executeRiskAnalysis(projectName, columns, context);
    ...
    try {
      ...
      console.error('Failed to parse AI JSON response in risk-analysis:', response.content, parseError);
      return NextResponse.json(
