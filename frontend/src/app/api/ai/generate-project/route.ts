...
export async function POST(req: NextRequest) {
  try {
    ...
    const { projectName, projectDescription, projectType, technologies, detailLevel, taskCount } = body;

    if (!projectDescription || !projectDescription.trim()) {
      return NextResponse.json(
        { error: 'Chybí popis nápadu projektu.' },
        { status: 400 }
      );
    }

    const response = await aiService.executeProjectGenerate(
      projectName || '',
      projectDescription,
      projectType || '',
      technologies || '',
    ...
    try {
      ...
      console.error('Failed to parse AI JSON response in generate-project:', response.content, parseError);
      return NextResponse.json(
